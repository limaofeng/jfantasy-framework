  ...
  static {
    try {
      ...
      String webAppRootKey =
          PropertiesHelper.load("plugin.properties").getProperty("webAppRootKey", "webapp.root");
      PathUtil.WEBROOT_PATH = System.getProperty(webAppRootKey);
      if (StringUtil.isNull(PathUtil.WEBROOT_PATH)) {
        log.debug("根据webAppRootKey[" + webAppRootKey + "]获取WebRoot路径失败");
        log.debug(
            "改用 Thread.currentThread().getContextClassLoader().getResource(\"/\") 方式获取WebRoot路径");
        URL url = Thread.currentThread().getContextClassLoader().getResource("");
        log.debug(
            "Thread.currentThread().getContextClassLoader().getResource(\"/\") 获取的 url = " + url);
        if (url == null) {
          url = PathUtil.class.getResource("");
          log.debug("PathUtil.class.getResource(\"\") 获取的 url = " + url);
        }
        assert url != null;
        ...
        if (WEBCLASSES_PATH.indexOf("WEB-INF") > 0) {
          ...
          PathUtil.WEBINF_PATH =
              RegexpUtil.replace(classFile.getParentFile().getPath(), "[\\/]$", "");
          PathUtil.WEBROOT_PATH =
              RegexpUtil.replace(classFile.getParentFile().getParentFile().getPath(), "[\\/]$", "");
        }
        if (StringUtil.isBlank(WEBROOT_PATH)
            || StringUtil.isBlank(WEBINF_PATH)
            || "/".equals(WEBCLASSES_PATH)) {
          ...
          if ("jar".equalsIgnoreCase(url.getProtocol())) {
            ...
            if (path.contains("WEB-INF")) {
              PathUtil.WEBROOT_PATH =
                  RegexpUtil.replace(path.substring(0, path.lastIndexOf("WEB-INF")), "[\\/]$", "");
        ...
        PathUtil.WEBROOT_PATH =
            StringUtil.defaultValue(PathUtil.WEBROOT_PATH, "")
                .replaceAll("\\" + File.separator, "/");
        PathUtil.WEBINF_PATH =
            StringUtil.defaultValue(PathUtil.WEBINF_PATH, "")
                .replaceAll("\\" + File.separator, "/");
  ...
  public static String getFilePath(String path) {
    ...
    if (path.endsWith(".xml")) {
      result
          .append(classes())
          .append("/")
          .append(path.replace(".xml", "").replace('.', '/').concat(".xml"));
    } else if (path.endsWith(".class")) {
      result
          .append(classes())
          .append("/")
          .append(path.replace(".class", "").replace('.', '/').concat(".class"));
    } else {
