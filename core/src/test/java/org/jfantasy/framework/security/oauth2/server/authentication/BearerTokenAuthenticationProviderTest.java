...
class BearerTokenAuthenticationProviderTest {
    ...
    @BeforeEach
    void setUp() {
        DefaultTokenServices defaultTokenServices = new DefaultTokenServices(new InMemoryTokenStore(), new DefaultClientDetailsService());
        defaultTokenServices.setTokenStore(new InMemoryTokenStore());
        BearerTokenAuthenticationProvider provider = new BearerTokenAuthenticationProvider(defaultTokenServices);
        manager.addProvider(provider);
    }

    @Test
    void authenticate() {
        WebAuthenticationDetails webAuthenticationDetails = authenticationDetailsSource.buildDetails(new MockHttpServletRequest());
