...
public class MailSendService implements InitializingBean {
  ...
  private static final Pattern PARSE_EMAIL =
      RegexpUtil.getPattern("^([\\S\\s]+)?\\<([\\S\\s]+)\\>");
  ...
  public void sendHtmlEmail(String title, String template, Object model, String... to) {
    try {
      ...
      email.setSubject(title);
      email.setContent(
          HandlebarsTemplateUtils.processTemplateIntoString(template, model),
  ...
  public void sendHtmlEmail(
      String title, String template, Object model, EmailAttachment[] attachs, String... to) {
    try {
      HtmlEmail email = (HtmlEmail) this.createEmail(EmailType.html, to);
      email.setSubject(title);
      email.setContent(
   ...
   */
  public void sendHtmlEmail(
      String title, String template, Object model, Attachment[] attachs, String... to) {
    try {
      HtmlEmail email = (HtmlEmail) this.createEmail(EmailType.html, to);
      email.setSubject(title);
      email.setContent(
      ...
      for (Attachment attachment : attachs) { // 添加流形式的附件
        if (attachment.getInputStream() != null) {
          ...
          try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            email.attach(
                new ByteArrayDataSource(out.toByteArray(), attachment.getContentType()),
                MimeUtility.encodeText(attachment.getName()),
                attachment.getDescription());
          }
        }
  ...
  public static class Attachment {
    ...
    public Attachment(
        InputStream inputStream, String contentType, String name, String description) {
    ...
    public Attachment(
        String template, Object model, String contentType, String name, String description) {
