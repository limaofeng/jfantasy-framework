...
import org.jfantasy.framework.util.regexp.RegexpUtil.ReplaceCallBack;
...
public class OraSQLDialect implements Dialect {
  ...
  protected static final String ORA_SQL_LIMIT =
      "select * from (select ora_a.*,rownum row_num from ({SQL}) ora_a ) ora_b where ora_b.row_num between {OFFSET} and {LIMIT}";
