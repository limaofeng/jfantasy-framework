...
public final class ImageUtil {
   ...
   * @throws IOException
   */
  public static BufferedImage watermark(String press, String target, int x, int y, int alpha)
      throws IOException {
   ...
   * @throws IOException
   */
  public static BufferedImage watermark(File press, File target, int x, int y, int alpha)
      throws IOException {
   ...
   * @throws IOException
   */
  public static BufferedImage watermark(String press, BufferedImage target, int x, int y, int alpha)
      throws IOException {
   ...
   * @throws IOException
   */
  public static BufferedImage watermark(BufferedImage press, File target, int x, int y, int alpha)
      throws IOException {
  ...
  public static BufferedImage watermark(
    ...
    Position position =
        watermarkPosition.position(srcWidth, srcHeight, watermarkImageWidth, watermarkImageHeight);
   ...
   */
  public static BufferedImage watermark(
      BufferedImage press, BufferedImage target, int x, int y, int alpha) {
  ...
  public static void write(BufferedImage image, String formatName, String filePath)
  ...
  public static BufferedImage pressText(
      String pressText,
      String targetImg,
      String fontName,
      int fontStyle,
   ...
   * @param width 压缩到的宽度
   * @param heigth 压缩到的高度 java.awt.image.BufferedImage <br>
   ...
   *     TYPE_INT_BGR 表示一个具有8位RGB颜色分量的图像，对应于Windows或Solaris风格的BGR颜色模型，具有打包为整数像素的Blue、Green和Red三种颜色。
   *     <br>
  ...
  public static BufferedImage reduce(
      BufferedImage imageOriginal, int realWidth, int realHeight, int width, int heigth) {
    try {
      int newWidth = 0;
      int newHeight = 0;
  ...
  public static String measure(String confine, String orig) {
    ...
    int newWidth = 0,
        realWidth = Integer.valueOf(origRatio[0]),
        width = Integer.valueOf(confineRatio[0]);
    int newHeight = 0,
        realHeight = Integer.valueOf(origRatio[1]),
        heigth = Integer.valueOf(confineRatio[1]);
    if ((realWidth >= width) && (width > 0)) {
      newWidth = width;
      newHeight = realHeight * newWidth / realWidth;
    }
  ...
  public static Image bmpReader(InputStream fs) throws IOException {
    ...
    try {
      ...
      int nheight =
          (bi[11] & 0xFF) << 24 | (bi[10] & 0xFF) << 16 | (bi[9] & 0xFF) << 8 | bi[8] & 0xFF;
      ...
      int nsizeimage =
          (bi[23] & 0xFF) << 24 | (bi[22] & 0xFF) << 16 | (bi[21] & 0xFF) << 8 | bi[20] & 0xFF;
  ...
  public static Image jpgReader(InputStream in) throws IOException {
    Toolkit kit = Toolkit.getDefaultToolkit();
    File file =
        new File(System.getProperty("java.io.tmpdir") + GUIDKeyGenerator.getInstance().getGUID());
  ...
  public static BufferedImage screenshots(BufferedImage img, int x, int y, int w, int h) {
    ...
    Image newImg =
        Toolkit.getDefaultToolkit()
            .createImage(new FilteredImageSource(img.getSource(), cropFilter));
  ...
  /**
   * 获取图片的编码字符串<br>
   * <img src="data:image/gif;base64,64位编码字符串"/>
   *
   * @param in 输入流
   * @return {String}
   */
  ...
  public enum WatermarkPosition {
    ...
    public Position position(
        int srcWidth, int srcHeight, int watermarkImageWidth, int watermarkImageHeight) {
      switch (this) {
        case topLeft:
        ...
        case center:
          return new Position(
              (srcWidth - watermarkImageWidth) / 2, (srcHeight - watermarkImageHeight) / 2);
        case bottomLeft:
