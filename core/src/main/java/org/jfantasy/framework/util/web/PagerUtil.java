...
public class PagerUtil {
  ...
  public static List<String> getPageArray(int curPage, int totalPage, int pageNumber) {
    ...
    for (int i = 1, size = 1; i <= totalPage && size <= (pageNumber * 2 + 3); i++) {
      if (i == 1) {
        arraylist.add(i + "");
        size++;
      } else if ((curPage > totalPage - (pageNumber + 1)
              && i + (pageNumber * 2 + 2) > totalPage) // 当前页在前5位
  ...
  public static String getPagerUrl(
      String requestUri, String queryString, String currentPage, String currentPageName) {
