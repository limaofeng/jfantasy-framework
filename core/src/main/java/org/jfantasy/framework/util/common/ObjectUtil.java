...
public final class ObjectUtil {
  ...
  public static <T, C extends Collection<T>> C filter(C list, String spel) {
    ...
    return filter(
        list, item -> expression.getValue(SpELUtil.createEvaluationContext(item), Boolean.class));
  }
  ...
  public static <T, C extends Collection<T>> int indexOf(
      C objs, Expression expression, Object value) {
    int i = 0;
    for (T obj : objs) {
  ...
  public static <T> List<T> sort(List<T> collectoin, String[] customSort, String idFieldName) {
    Collections.sort(
        collectoin instanceof PersistentBag ? new ArrayList<>(collectoin) : collectoin,
        new CustomSortOrderComparator(customSort, idFieldName));
    return collectoin;
  }
