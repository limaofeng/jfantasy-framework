...
import net.asany.jfantasy.framework.log.LogManager;
import net.asany.jfantasy.framework.log.annotation.CompositeLogOperationSource;
import net.asany.jfantasy.framework.log.annotation.LogOperation;
...
public class LogAspectSupport implements InitializingBean {
  ...
  protected Object execute(Invoker invoker, Object target, Method method, Object[] args) {
    ...
    Class<?> targetClass =
        ObjectUtil.defaultValue(AopProxyUtils.ultimateTargetClass(target), target.getClass());
    final Collection<LogOperation> logOp =
        getLogOperationSource().getOperations(method, targetClass);
  ...
  private Collection<LogOperationContext> createOperationContext(
      Collection<LogOperation> logOp,
      Method method,
      Object[] args,
      Object target,
      Class<?> targetClass) {
    ...
    for (LogOperation logOperation : logOp) {
      LogOperationContext opContext =
          getOperationContext(logOperation, method, args, target, targetClass);
  ...
  protected LogOperationContext getOperationContext(
      LogOperation operation, Method method, Object[] args, Object target, Class<?> targetClass) {
  ...
  protected class LogOperationContext {
    ...
    protected boolean isConditionPassing() {
      return !StringUtils.hasText(this.operation.getCondition())
          || evaluator.condition(this.operation.getCondition(), this.method, this.evalContext);
    }
