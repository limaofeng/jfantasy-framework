...
import net.asany.jfantasy.framework.dao.mybatis.dialect.Dialect;
...
public class LimitInterceptor implements Interceptor {
  ...
  private Object processIntercept(Invocation invocation, final Object[] queryArgs)
    ...
    page.reset(executeForList(invocation, ms, parameter));
  ...
  private int executeForCount(MappedStatement ms, Map<String, Object> parameter) {
    ...
    try {
      ...
      connection =
          newMappedStatement.getConfiguration().getEnvironment().getDataSource().getConnection();
      ParameterHandler dp =
          new DefaultParameterHandler(ms, parameter, sqlSource.getBoundSql(parameter));
   ...
   * @return SqlSource
   */
  private SqlSource getCountSqlSource(
      MappedStatement mappedStatement, Map<String, Object> parameterObject) {
   ...
   * @return SqlSource
   */
  private SqlSource getPageLimitSqlSource(
      MappedStatement mappedStatement, Map<String, Object> parameterObject) {
    SqlSourceBuilder sqlSourceParser = new SqlSourceBuilder(mappedStatement.getConfiguration());
    String mapperSQL = getMapperSQL(mappedStatement, parameterObject);
  ...
  private String getMapperSQL(MappedStatement mappedStatement, Object parameterObject) {
    ...
    if (nowSqlSource instanceof DynamicSqlSource) {
      ...
      DynamicContext context =
          new DynamicContext(mappedStatement.getConfiguration(), parameterObject);
      ...
      if (ClassUtil.getValue(nowSqlSource, SQL_SOURCE_NAME) instanceof StaticSqlSource) {
        ...
        final List<ParameterMapping> parameterMappings =
            ClassUtil.getValue(ClassUtil.getValue(nowSqlSource, "sqlSource"), "parameterMappings");
        return RegexpUtil.replace(
            sql,
            "\\?",
            (text, index, matcher) -> "#{" + parameterMappings.get(index).getProperty() + "}");
      }
  ...
  private MappedStatement copyMappedStatementBySqlSource(
    ...
    if (ObjectUtil.isNotNull(mappedStatement.getKeyProperties())
        && mappedStatement.getKeyProperties().length > 0) {
