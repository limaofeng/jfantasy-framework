...
import net.asany.jfantasy.graphql.gateway.execution.AuthenticationGraphQLException;
...
public class HttpRequestInvokerImpl implements HttpRequestInvoker {
  ...
  private void invokeAndHandleAsync(
    ...
    AsyncTimeoutListener timeoutListener =
        event -> {
          log.warn(
              "GraphQL execution canceled because timeout of {} millis was reached. The following query was being executed when this happened:\n{}",
  ...
  private ExecutionResult toErrorResult(Throwable t) {
    if (t instanceof AuthenticationGraphQLException e) {
      ErrorContext errorContext =
          new ErrorContext(
              new ArrayList<>(),
              new ArrayList<>(e.getPath().getKeysOnly()),
              new HashMap<>(),
              ErrorType.DataFetchingException);
      DefaultGraphQLError error = GraphQLErrorUtils.buildGraphqlError(errorContext, e);
      return new ExecutionResultImpl(error);
    }
    ...
    GraphQLError graphQLError = new GenericGraphQLError(message);

    return new ExecutionResultImpl(graphQLError);
  }
